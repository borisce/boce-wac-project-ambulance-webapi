...
type DbService[DocType interface{}] interface {
	...
	GetAllAppointments(ctx context.Context) ([]DocType, error) // Add this line
}
...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.DbName == "" {
		svc.DbName = enviro("AMBULANCE_API_MONGODB_DATABASE", "boce-wac-project-ambulance-wl")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("AMBULANCE_API_MONGODB_COLLECTION", "appointmentslist")
	}
...
func (this *mongoSvc[DocType]) GetAllAppointments(ctx context.Context) ([]DocType, error) {
    ctx, contextCancel := context.WithTimeout(ctx, this.Timeout)
    defer contextCancel()
